...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.wntf8.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.get("/available", async (req, res) => {
      ...
      //step3: for each service find service  for that service
      services.forEach((service) => {
        //step4: find bookings for that service
        const serviceBookings = bookings.filter(
          (book) => book.treatment === service.name
        );
        //selects slots for the service bookings
        const bookedSlot = serviceBookings.map((book) => book.slot);
        //step6: select those slots that are not in bokslots
